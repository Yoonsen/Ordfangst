...
def adjust(df, date, days, smooth):
    ...
    try:
        ...
        s = pd.Timestamp(
            min(
                pd.Timestamp("20210701"),
                pd.Timestamp((ts - pd.Timedelta(days=days + min_days))),
            ).strftime("%Y%m%d")
        )
        e = pd.Timestamp(
            min(pd.Timestamp.today(), pd.Timestamp((ts + td))).strftime("%Y%m%d")
        )
...
st.set_page_config(
    page_title="Dagsplott", layout="wide", initial_sidebar_state="auto", menu_items=None
)
...
with colw:
    words = st.text_input(
        "Enkeltord",
        "frihet",
        help="Skriv inn ett eller flere enkeltord adskilt med komma. Det er forskjell på store og små bokstaver",
    )
    allword = list(set([w.strip() for w in words.split(",")]))[:30]

with cols:
    sammenlign = st.text_input(
        "Relativt til",
        ". ,",
        help="Sammenlingn med summen av en liste med ord. Om listen ikke er tom viser y-aksen antall forekomster pr summen av ordene det sammenlignes med. Er listen tom viser y-aksen det absolutte antall forekomster ",
    )

with cola:
    avisnavn = st.selectbox(
        "Velg avis",
        titles(),
        index=0,
        help="Begynn å skrive navnet på en avis, og velg fra listen, om ingen avis velges aggregreres over alle",
    )
    if avisnavn == "--ingen--":
        avisnavn = None
...
with coldmidt:
    mid_date = st.date_input(
        "Dato",
        value=last_date - datetime.timedelta(days=int(max_days / 2)),
        min_value=datetime.date(179, 1, 1),
        max_value=datetime.date.today(),
    )
with coldperiode:
    period_size = st.number_input(
        f"Periode",
        min_value=min_days,
        max_value=max_days,
        value=max_days,
        step=100,
        help="Lengde på periode i antall dager, maks {max_days}, minimum {min_days}",
    )
    start_date = min(
        datetime.date.today() - datetime.timedelta(days=2),
        mid_date - datetime.timedelta(days=period_size),
    )
    end_date = min(
        datetime.date.today(), mid_date + datetime.timedelta(days=period_size)
    )
...
def show_data(data, alfa=0.9, width=0.8):
    ...
    plt.rcParams.update(
        {"font.size": 8, "font.family": "monospace", "font.monospace": "Courier"}
    )
...
with colf:
    filnavn = st.text_input(
        ...
        "dagsplott.xlsx",
        help="Filen blir sannsynligvis liggende i nedlastningsmappen - endre gjerne på filnavnet, men behold .xlsx",
    )

if st.download_button(
    f"Klikk for å laste ned til {filnavn}",
    to_excel(df),
    filnavn,
